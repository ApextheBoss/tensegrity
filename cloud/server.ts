...
function readBody(req: IncomingMessage): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', (c: Buffer) => { console.log('data chunk:', c.length); chunks.push(c); });
...
function extractApiKey(req: IncomingMessage): string | null {
  ...
  if (auth?.startsWith('Bearer tsg_')) return auth.slice(7);
  return null;
}

function refreshAgentStatuses(ws: Workspace): void {
  const now = Date.now();
  for (const agent of ws.agents.values()) {
    if (agent.status === 'disconnected') continue;
    const elapsed = now - agent.lastHeartbeat;
    if (elapsed > 60000) agent.status = 'disconnected';
    else if (elapsed > 30000) agent.status = 'degraded';
  }
}
...
async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (!apiKey) return json(res, { error: 'Missing API key' }, 401);
  const ws = getWorkspaceByKey(apiKey);
  if (!ws) return json(res, { error: 'Invalid API key' }, 401);
  ...
  if (agentDeleteMatch && method === 'DELETE') {
    if (!ws.agents.has(agentDeleteMatch[1])) return json(res, { error: 'Agent not found' }, 404);
  ...
  if (path === '/api/route' && method === 'POST') {
    ...
    const candidates = Array.from(ws.agents.values()).filter(a =>
      a.status === 'connected' && a.capabilities.includes(body.capability)
    );
    if (candidates.length === 0) return json(res, { error: 'No available agents for: ' + body.capability }, 503);
  ...
  if (completeMatch && method === 'POST') {
    ...
    if (task) { task.status = 'completed'; task.latencyMs = body.latencyMs; }
